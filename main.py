# input
ciphers = [
  '',
  '234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f',
  '32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb',
  '',
  '3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070',
  '',
  '',
  '315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3',
  '',
  '466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83',
]
target = '32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904'

# solution
answer = ['.'] * (len(bytes.fromhex(target)))
max_len = len(max(ciphers, key=lambda x: len(x))) // 2
is_space = [[0] * max_len for _ in range(11)]

def xor_ciphers(cipher1, cipher2):
  bytes1 = bytes.fromhex(cipher1)
  bytes2 = bytes.fromhex(cipher2)
  xored_bytes = bytes([a ^ b for a, b in zip(bytes1, bytes2)])
  return xored_bytes

def is_alpha(byte):
  return 65 <= byte <= 90 or 97 <= byte <= 122

# for i in range(1, len(ciphers)):
#   for j in range(i):
#     byte_str = xor_ciphers(ciphers[i], ciphers[j])
#     for idx, byte in enumerate(byte_str):
#       if is_alpha(byte):
#         is_space[i][idx] += 1
#         is_space[j][idx] += 1

# target_bytes = bytes.fromhex(target)

# for i, byte in enumerate(target_bytes):
#   max_spaces = 0
#   for j in range(len(ciphers)):
#     if is_space[j][i] > max_spaces:
#       char = xor_ciphers(ciphers[j], target)[i]
#       if is_alpha(char):
#         answer[i] = chr(char)

def find_chars(cipher, idx):
  byte1 = xor_ciphers(cipher, ciphers[idx])
  byte2 = xor_ciphers(cipher, target)
  list1 = []
  for i, byte in enumerate(byte1):
    if is_alpha(byte):
      list1.append(i)
  for i, byte in enumerate(byte2):
    if is_alpha(byte) and i in list1 and answer[i] == '.':
      answer[i] = chr(byte)

for i in range(len(ciphers)):
  find_chars(ciphers[i], 2)

# with open("output.txt", 'a') as file:
#   file.write(''.join(answer) + '\n')

# -> the message is "The secret message is when using a stream cipher never use the key more than once"

# modify the message to decrypt some the other ciphertexts too
plaintext = 'The secret message is  When using a stream cipher  never use the key more than once'
plaintext_hex = ''.join(hex(ord(c))[2:] for c in plaintext)

key = xor_ciphers(plaintext_hex, target)
cipher1 = bytes.fromhex(ciphers[1])
result = bytes([a ^ b for a, b in zip(key, cipher1)])
print(result.decode('ascii'))